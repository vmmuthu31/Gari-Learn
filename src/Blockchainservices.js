...
const connectWalletRenderPopup = async () => {
  //first time users are connecting to wallet this function will activate
  try {
    setLoading(true);
    if (solana) {
      const response = await solana.connect();
...
const connect = () => {
  return (
    <button onClick={connectWalletRenderPopup} className="buttonStyle">
      {" "}
      {Loading ? <p>loading...</p> : <p>Connect Your Wallet To Post </p>}{" "}
    </button>
  );
};

const createPostFunction = async (text, hastag, position) => {
  //createPostFunction connects to the smartcontract via rpc and lib.json  to create post
  ...
  try {
    ...
    Promise.all(
      ...
      result.sort(function (a, b) {
        return b.position.words[0] - a.position.words[0];
      });
      setData([...result]);
    });
  } catch (err) {
